<template>
    <v-app class="app_works">
        <div class="works_items" >
            <template v-if="loading || !parentAnimationCompleted">
                <div class="works_items_box loading_box" v-for="n in 6" :key="n">
                    <div class="loading_box_title"></div>
                    <div class="loading_box_techno"></div>
                </div>
            </template>
            <nuxt-link v-show="isAllImagesLoaded && parentAnimationCompleted" :to="'/works/'+work.id" class="works_items_box" v-for="(work,index) in works" :key="work.id">
...
export default {
    ...
    data() {
        return {
            ...
            imageLoaded : null,
            loading : true,
            parentAnimationCompleted : false,
        }
    },
    methods : {
        imageLoad(index) {
